...
import { initDb } from './loader/sequelize';
import { configExpress, configExpressError, configExpressNotFoundError } from './loader/express';
...
process.on('unhandledRejection', (reason: string | null | undefined, promise: Promise<any>) => {
...
async function serve(): Promise<void> {
  ...
  const app = new InversifyExpressServer(container, null, null, null, null)
    .setConfig(configExpress)
    .setErrorConfig(configExpressError)
    .build();
  ...
  server.listen(SERVER_PORT, () => {
    logger.info('SERVER_STARTED_ON_PORT: %o', (server.address() as AddressInfo).port);
  });
}
